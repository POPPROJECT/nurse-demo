...
function FilterBar({
    ...
    const selectedBookOption = bookOptions.find((opt) => opt.value === selectedBook) || null;
    const selectedCourseOption = courseOptions.find((opt) => opt.value === selectedCourse) || null;
        ...
        <div className="p-4 mb-6 bg-white shadow rounded-xl dark:bg-[#1E293B]">
            <div className="flex flex-col items-center justify-center w-full gap-4 md:flex-row">
                ...
                <div className="flex-1 w-full">
                    <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-white">สมุด</label>
                    <Select
                ...
                {/* Course Select */}
                <div className="flex-1 w-full">
                    <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-white">รายวิชา</label>
                    <Select
                        instanceId="course-select-subject-dashboard"
                        options={courseOptions}
                        value={selectedCourseOption}
                        onChange={(opt) => setSelectedCourse(opt ? opt.value : '')}
                        placeholder={isCourseLoading ? 'กำลังโหลด...' : '-- เลือกรายวิชา --'}
                        isClearable
...
function ProgressDonut({ chartId, percentage, label }: { chartId: string; percentage: number; label: string }) {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const chartRef = useRef<Chart | null>(null);

    const getColor = (p: number) => {
        if (p >= 100) return '#22c55e'; if (p >= 80) return '#3b82f6'; if (p >= 60) return '#8b5cf6';
        if (p >= 40) return '#f59e0b'; if (p >= 20) return '#f97316'; return '#ef4444';
    };
    ...
    return (
        <div className="flex flex-col items-center p-4 bg-white border border-gray-200 shadow dark:bg-[#1E293B] rounded-xl dark:border-gray-700 hover:shadow-md">
            ...
            <h3 className="mb-1 text-base font-medium text-center text-gray-800 dark:text-white">{label}</h3>
        </div>
    );
}
// --- END SUB-COMPONENTS ---
...
export default function DashboardSubjectPage() {
    ...
    useEffect(() => {
        if (!accessToken) { setLoadingBooks(false); return; }
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setPageError(null);
        fetch(`${BACKEND_URL}/approver/dashboard-subject?bookId=${bookId}&courseId=${courseId}`, {
            headers: { Authorization: `Bearer ${accessToken}` },
        })
            .then((res) => {
    ...
    // --- RENDER LOGIC ---
    if (loadingBooks) return <div className="p-10 text-center">Loading Books...</div>;
    if (pageError) return <div className="p-10 text-center text-red-500">Error: {pageError}</div>;

    const selectedCourseName = courses.find(c => c.id === courseId)?.name || 'รายวิชา';
    ...
    return (
        <div className="container max-w-6xl px-4 py-8 mx-auto mt-10 sm:mt-0">
            ...
            <div className="p-6 mb-6 text-white bg-[linear-gradient(to_right,#f46b45_0%,#eea849_100%)] dark:bg-[#1E293B] rounded-xl shadow-md">
                <h1 className="text-xl font-semibold sm:text-2xl">ภาพรวมความคืบหน้ารายวิชา</h1>
            </div>
            ...
            {/* Conditional Content */}
            {loading && <p className="py-10 text-center text-gray-500">กำลังโหลดข้อมูล...</p>}

            {!loading && (!bookId || !courseId) && (
                <p className="py-10 text-center text-gray-500">กรุณาเลือกสมุดและรายวิชาเพื่อแสดงข้อมูล</p>
            )}
