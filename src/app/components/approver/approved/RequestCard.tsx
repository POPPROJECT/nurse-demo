...
type Props = {
  ...
  onConfirm: (pin?: string) => Promise<void>; // ✅ เปลี่ยนจาก void เป็น Promise<void>
  onReject: (pin?: string) => Promise<void>; // ✅ เปลี่ยนจาก void เป็น Promise<void>
    ...
    <div className="relative bg-white dark:bg-[#1E293B] hover:-translate-y-1 hover:shadow-lg transition-all dark:text-white rounded-xl shadow overflow-hidden">
      ...
      <div className="p-6 mt-4">
        ...
        <div className="flex flex-col pt-4 mt-4 space-y-2 border-t sm:flex-row sm:space-y-0 sm:space-x-2 sm:justify-end">
          ...
          <button
            onClick={() => onConfirm()} // ไม่ต้องส่ง pin ถ้า Swal.fire ใน Page Component จัดการเรื่อง PIN เอง
          ...
          {/* {req.status === 'PENDING' && ( */}
          <button
            onClick={() => onReject()} // ไม่ต้องส่ง pin ถ้า Swal.fire ใน Page Component จัดการเรื่อง PIN เอง
